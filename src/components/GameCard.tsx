  ...
  return (
    <>
      ...
      >
        ...
        >
          <span className="box-border block overflow-hidden w-[initial] h-[initial] bg-none opacity-100 border-none m-0 p-0 relative">
            <span className="box-border block w-[initial] h-[initial] bg-none opacity-100 border-none m-0 pt-[46.7391%] pb-0 px-0">
              <img className="p-0 block m-auto min-w-full max-w-full max-h-full min-h-full inset-0 w-auto h-auto absolute"
          ...
          {moreInfo && (
            <div className="relative bg-[#6E644E] rounded-xl mt-11 p-2 -left-1/2 top-6 w-screen md:w-full">
              {name === 'Skyrim' ? (
                ...
                <img className="m-auto" alt={`${name} stats`} src={stats} />
              )}
            </div>
          )}
        </div>
...
function Skyrim({ stats }: skyrimProps) {
  return (
    <>
      ...
      <caption className="mt-5 font-bold">Elder Scrolls Favorites</caption>
        <tbody>
