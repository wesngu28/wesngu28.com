...
export const GET: APIRoute = async () => {
  ...
  while (Object.keys(artists).includes('message')) {
    const response = await fetch(
      `https://ws.audioscrobbler.com/2.0/?method=user.gettopartists&user=scrambledeggdog&period=1month&limit=8&api_key=${LASTFM_API_KEY}&format=json`
    )
